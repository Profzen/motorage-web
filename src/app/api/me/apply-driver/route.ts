...
export async function POST(request: Request) {
    try {
        ...
        if (existingApplication && existingApplication.statut === 'en_attente') {
            return ApiErrors.badRequest('Une demande est déjà en cours de traitement');
        }
        ...
        // Créer la demande
        const newApplication = await db.insert(onboardingRequests).values({
        ...
        return successResponse({
            message: "Votre demande a été soumise avec succès et est en attente de validation.",
